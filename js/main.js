const name = [
  'Соня',
  'Алиса',
  'Василий',
  'Олег',
  'Лариса',
  'Лев',
  'Надежда',
  'Любовь',
  'Вера',
  'Костя',
];

const description = [
  'Очень красиво получилось!',
  'Ракета, бомба, петарда!',
  'Годная темка',
  'Я как всегда шикарен!',
  'А это платно?',
  'Яб тоже хотел так.',
  'Пока это лучшее, что я видел',
  'Как вам это?',
  'Я бы немного пересмотрел данное действие',
  'Ну вообще круто, но не очень',
];

const message = [
  'Всё отлично!',
  'В целом всё неплохо. Но не всё.',
  'Когда вы делаете фотографию, хорошо бы убирать палец из кадра. В конце концов это просто непрофессионально.',
  'Моя бабушка случайно чихнула с фотоаппаратом в руках и у неё получилась фотография лучше.',
  'Я поскользнулся на банановой кожуре и уронил фотоаппарат на кота и у меня получилась фотография лучше.',
  'Лица у людей на фотке перекошены, как будто их избивают. Как можно было поймать такой неудачный момент?!',
];

// Функция для рандомного количества лайков
const generateRandomLikes = function () {
  return Math.floor(Math.random() * (200 - 15 + 1)) + 15;
};
const randomLikes = generateRandomLikes();

// Функция для рандомного количества комментов
const generateRandomComments = function () {
  return Math.floor(Math.random() * (30 - 0 + 1));
};
const randomComments = generateRandomComments();

